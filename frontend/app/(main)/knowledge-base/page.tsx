...
function KnowledgeBasePageContent() {
  ...
  const handleGoToCollection = useCallback(() => {
    ...
    // KB collection roots are 'kb' nodeType — 'recordGroup' caused 404 here too.
    const urlParams = new URLSearchParams();
    urlParams.set('nodeType', 'kb');
    ...
    async (name: string, description: string) => {
      ...
      try {
        if (createFolderContext?.type === 'subfolder' && createFolderContext.kbId) {
          ...
          // Newly created KBs are root-level 'kb' nodes, not 'recordGroup'.
          // 'recordGroup' is a child layer inside a KB — using it here caused a 404.
          router.push(buildNavUrl({ nodeType: 'kb', nodeId: newCollection.id }));
